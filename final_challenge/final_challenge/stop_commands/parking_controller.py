...
from fc_msgs.msg import StopLightLocation,StopLightPixel, ParkingError
...
class ParkingController(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(StopLightLocation, "/relative_stoplight",
            self.stoplight_callback, 1)
    ...
    def stoplight_callback(self, msg):
        """
        Given the stop light location msg, in terms of real life coordinates x, y, publish a drive command
        """
        self.relative_x = msg.x_pos
        self.relative_y = msg.y_pos

        # Not sure if we still need angle correction
        ...
        if abs(self.dist - self.parking_distance) < dist_eps and abs(angle-angle_des) < angle_eps:
            ...
            if self.parking_distance>self.dist or self.distance_check:
                #if P>0, we are too close to the stop light and may need to back up
                ...
                if self.relative_x < 0:
                    #if the stop light  is behind us, go backward instead of forward
                    #we also don't want to use pure pursuit as we will just drive
                    #backward into stop light , so just back up and correct angle
                    speed = -speed
    ...
    def error_publisher(self):
        """
        Publish the error between the car and the stop light. We will view this
